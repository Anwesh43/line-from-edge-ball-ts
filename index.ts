...
class LFECNode {
    ...
    addNeighbor() {
        if (this.i < colors.length - 1) {
            this.next = new LFECNode(this.i + 1)
            this.next.prev = this 
        }
    }

    draw(context : CanvasRenderingContext2D) {
        DrawingUtil.drawLFECNode(context, this.i, this.state.scale)
    }
    ...
    getNext(dir : number, cb : Function) : LFECNode {
        var curr : LFECNode = this.prev 
        if (dir == 1) {
            curr = this.next 
        }
        if (curr) {
...
class LineFromEdgeCircle {

    curr : LFECNode = new LFECNode(0)
    dir : number = 1
